 ...
 * @returns
 */
export const addMemoryCache = (cache) => {
    const isOk = memoryCacheList.addCache(cache);
    if (!isOk)
 ...
 * @returns
 */
export const getMemoryCache = (cacheName) => {
 ...
 * @returns
 */
export const addSingleMemoryCache = (cacheName, singleCache) => {
 ...
 * @returns
 */
export const addSingleMemoryCacheList = (cacheName, singleCacheList) => {
    const _memoryCache = memoryCacheList.getCache(cacheName);
 ...
 * @returns
 */
export const getSingleMemoryCache = (cacheName, cacheId) => {
    const _memoryCache = memoryCacheList.getCache(cacheName);
    ...
    if (_singleCache === null)
        return null;
    //判断数据是否超时
    if (_memoryCache.readLimitTime !== 0 &&
 ...
 * @returns
 */
export const getSingleMemoryCacheCons = (cacheName, cacheId) => {
    const _memoryCache = memoryCacheList.getCache(cacheName);
    const _singleCache = _memoryCache.getSingleCache(cacheId);
    //判断是否为空
    if (_singleCache === null)
    ...
    //判断是否超出读取次数了
    if (_memoryCache.readLimitNumber !== 0 && _singleCache._readNumber === 1)
        _memoryCache.delSingleCache(cacheId);
    else if (_memoryCache.readLimitNumber !== 0 && _singleCache._readNumber > 1)
        _singleCache._readNumber--;
    return _singleCache;
};
